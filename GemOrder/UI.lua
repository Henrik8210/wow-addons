...
local function ColorizeGem(name)
    local color = GEM_COLORS[GemOrder_GetGemColor(name)] or GEM_COLORS.Unknown
    return string.format("|cff%s%s|r", color, name)
end
...
local function LayoutQueueScrollbarOnce(inset)
    local scrollBar = _G["GemOrderScrollScrollBar"]
    if scrollBar then
...
local function CreateMainFrameHelper()
    local ok, frame = pcall(CreateFrame, "Frame", "GemOrderFrame", UIParent, "PortraitFrameTemplate")
    if ok and frame then
        return frame
    end
    frame = CreateFrame("Frame", "GemOrderFrame", UIParent)
    if frame.SetBackdrop then
        frame:SetBackdrop({
            bgFile = "Interface\\DialogFrame\\UI-DialogBox-Background",
...
local function HideOrderDialogSilently()
    if not GemOrder.UI or not GemOrder.UI.frame then
        return
    end
    local f = GemOrder.UI.frame
...
local function GetAddonVersion()
    if GemOrder_GetVersion then
        return GemOrder_GetVersion()
    end
...
function UI:BuildDropdownCaches()
    ...
    for _, gem in ipairs(GemOrder_GetCutGems()) do
...
function UI:RefreshJoinDropdownCache()
    self.joinRoomOptions = GemOrder_GetOpenRooms()
end
...
function UI:CreateMainFrame()
    ...
    GemOrder_RegisterEscapeFrame(f)
    ...
    f.creditsDev = CreateLabel(f, "Developed by " .. GemOrder_ColorizePlayer("Nobunda", "SHAMAN"), "GameFontDisableSmall")
    f.creditsDev:SetPoint("BOTTOM", 0, 22)
    f.creditsDev:SetJustifyH("CENTER")

    f.creditsTest = CreateLabel(f, "Tested by " .. GemOrder_ColorizePlayer("Justtwo", "WARLOCK"), "GameFontDisableSmall")
...
function UI:UpdateTabAccess()
    local joined = GemOrder_HasJoinedWorkshop()
    local f = self.frame
...
function UI:ShowTab(tabId, skipAccessCheck)
    ...
    if not skipAccessCheck and (tabId == "orders" or tabId == "completed" or tabId == "stock" or tabId == "recipes") and not GemOrder_HasJoinedWorkshop() then
        tabId = "workshop"
    end
    ...
    if (tabId == "orders" or tabId == "completed" or tabId == "recipes") and GemOrder_HasJoinedWorkshop() and IsInGuild() then
...
function UI:CreateWorkshopPanel()
    ...
    panel.createBtn:SetScript("OnClick", function()
        local _, err = GemOrder_CreateWorkshop(panel.nameInput:GetText())
        if err then
            print("|cff00ccffGemOrder|r " .. err)
        else
            panel.nameInput:SetText("")
            print("|cff00ccffGemOrder|r Workshop created.")
            self:Refresh()
        end
    end)
    ...
    panel.joinDropdown = CreateFrame("Frame", "GemOrderJoinDropdown", panel, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(panel.joinDropdown, function()
        GemOrder_ClearDropdownItemTooltips()
        ...
        for _, openRoom in ipairs(UI.joinRoomOptions or {}) do
            ...
            MarkDropdownSelection(info, GemOrder_GetActiveRoomId() == openRoom.id)
            info.func = function()
                local _, err = GemOrder_JoinWorkshop(openRoom.id)
                if err then
                    print("|cff00ccffGemOrder|r " .. err)
                else
                    print("|cff00ccffGemOrder|r Selected " .. openRoom.name .. ".")
    ...
    panel.changeWorkshopBtn:SetScript("OnClick", function()
        SafeCloseDropdownMenus()
        local _, err = GemOrder_DeselectWorkshop()
        if err then
            print("|cff00ccffGemOrder|r " .. err)
        else
            print("|cff00ccffGemOrder|r Workshop deselected. You remain a member — select another below.")
    ...
    local scroll = CreateFrame("ScrollFrame", "GemOrderWorkshopMembersScroll", panel, "UIPanelScrollFrameTemplate")
...
function UI:RefreshWorkshopMembers()
    ...
    local isManager = GemOrder_CanManageWorkshop(room, player)
    local isOwner = GemOrder_IsWorkshopOwner(room, player)
    local members = GemOrder_GetSortedRoomMembers(room)
    local y = 0
    for _, name in ipairs(members) do
        ...
        label:SetText(GemOrder_ColorizePlayer(name) .. tagText)
        ...
        if canManageJC or canManageCoLeader then
            ...
            if canManageCoLeader and not isTargetCoLeader then
                placeBtn("Make co-leader", 124, function()
                    local ok, err = GemOrder_AddCoLeader(room.id, name)
                    if not ok then
                        print("|cff00ccffGemOrder|r " .. (err or "Action failed."))
                    else
                        self:Refresh()
                    end
                end)
            ...
            if canManageJC then
                if isJewelcrafter then
                    placeBtn("Demote JC", 88, function()
                        local ok, err = GemOrder_DemoteCollaborator(room.id, name)
                        if not ok then
                            print("|cff00ccffGemOrder|r " .. (err or "Action failed."))
                        else
                            self:Refresh()
                    ...
                    placeBtn("Make JC", 88, function()
                        local ok, err = GemOrder_PromoteMember(room.id, name)
                        if not ok then
                            print("|cff00ccffGemOrder|r " .. (err or "Action failed."))
                        else
                            self:Refresh()
            ...
            if canManageCoLeader and isTargetCoLeader then
                placeBtn("Demote co-leader", 124, function()
                    local ok, err = GemOrder_RemoveCoLeader(room.id, name)
                    if not ok then
                        print("|cff00ccffGemOrder|r " .. (err or "Action failed."))
                    else
                        self:Refresh()
...
function UI:RefreshWorkshopPanel()
    ...
    if room then
        ...
        if room.leader == player then
            role = "Leader"
        elseif GemOrder_IsRoomCoLeader(room, player) then
            role = "Co-leader"
        end
        ...
        panel.status:SetText(string.format(
            ...
            room.name,
            GemOrder_ColorizePlayer(room.leader),
            role
        ))
    else
    ...
    elseif #GemOrder_GetOpenRooms() == 0 then
    ...
    local canManageWorkshop = room and GemOrder_CanManageWorkshop(room, UnitName("player"))
...
function UI:CreateStockPanel()
    ...
    local scroll = CreateFrame("ScrollFrame", "GemOrderStockScroll", panel, "UIPanelScrollFrameTemplate")
...
function UI:RefreshStockPanel()
    ...
    local function AddGemLines(counts, indent)
        indent = indent or 16
        local listing = GemOrder_GetStockListing(counts)
        if #listing == 0 then
        ...
        for _, entry in ipairs(listing) do
            ...
            if entry.itemId then
                GemOrder_AttachItemTooltip(row, function()
                    return entry.itemId
                end)
            end
    ...
    local function AddSection(title, counts, style, opts)
        ...
        elseif style == "jc" then
            ...
            local nameText = GemOrder_ColorizePlayer(title)
    ...
    local player = UnitName("player")
    local canShare = GemOrder_ShouldShareStock()
    if canShare then
        local personalTitle = "Your Bags + Bank"
        local bankNote = GemOrder_GetBankStockNote and GemOrder_GetBankStockNote()
        if bankNote then
            personalTitle = personalTitle .. " |cff888888(" .. bankNote .. ")|r"
        end
        AddSection(personalTitle, GemOrder_GetStockCounts("personal"), "personal")
    end
    ...
    AddSection("Workshop Total", GemOrder_GetAggregatedWorkshopStock(room), "total")
...
function UI:CreateRecipesPanel()
    ...
    panel.refreshBtn = CreateFrame("Button", "GemOrderRecipesRefresh", panel, "SecureActionButtonTemplate, UIPanelButtonTemplate")
    panel.refreshBtn:SetSize(100, 22)
    panel.refreshBtn:SetPoint("TOPRIGHT", -16, -14)
    panel.refreshBtn:SetText("Refresh")
    panel.refreshBtn:SetScript("OnShow", function(self)
        if GemOrder_InitRecipesRefreshButton then
            GemOrder_InitRecipesRefreshButton(self)
        end
    end)

    local scroll = CreateFrame("ScrollFrame", "GemOrderRecipesScroll", panel, "UIPanelScrollFrameTemplate")
...
function UI:RefreshRecipesPanel()
    ...
    local y = 8
    for _, entry in ipairs(GemOrder_GetRecipeCoverage(room, tier)) do
        local gem = entry.gem
        if gem.color ~= currentColor then
        ...
        GemOrder_AttachItemTooltip(nameBtn, function()
            return gem.itemId
        end)
        ...
        if hasRecipe then
            ...
            for _, jcName in ipairs(entry.jcs) do
                table.insert(names, GemOrder_ColorizePlayer(jcName))
            end
...
function UI:PopulateGearDropdown(dropdown, frameRef)
    ...
    if not category then
        ...
        info.notCheckable = true
        UIDropDownMenu_AddButton(info)
        GemOrder_ApplyDropdownItemTooltips(indexToItemId)
        return
    end

    if category == "PVE" then
        for _, raid in ipairs(GemOrder_GearRaids) do
            ...
            for _, gear in ipairs(GemOrder_GetGearByRaid(raid)) do
                local info = UIDropDownMenu_CreateInfo()
        ...
        for _, gear in ipairs(GemOrder_GetGearByClass(className)) do
            local info = UIDropDownMenu_CreateInfo()
            info.text = gear.label
            info.value = gear.itemId
            MarkDropdownSelection(info, frameRef.selectedGear and frameRef.selectedGear.itemId == gear.itemId)
...
function UI:CreateGearDropdown(name, parent, point, x, y, frameRef, width)
    ...
    GemOrder_AttachDropdownTooltips(dropdown, function()
...
function UI:CreateGearTypeDropdown(name, parent, point, x, y, frameRef, width)
    ...
    UIDropDownMenu_Initialize(dropdown, function()
        ...
        for _, category in ipairs(GemOrder_GearCategories) do
            local info = UIDropDownMenu_CreateInfo()
            info.text = category
...
function UI:CreateGearClassDropdown(name, parent, point, x, y, frameRef, width)
    ...
    UIDropDownMenu_Initialize(dropdown, function()
        ...
        for _, className in ipairs(GemOrder_PvpClasses) do
            local info = UIDropDownMenu_CreateInfo()
            info.text = className
...
function UI:CreateGemDropdown(name, parent, point, x, y, onSelect, getSelected, width)
    ...
    UIDropDownMenu_Initialize(dropdown, function()
        ...
        for _, entry in ipairs(self.gemMenuEntries) do
            ...
            elseif entry.kind == "gem" then
                local gem = entry.gem
                local label = GemOrder_FormatGemDropdownLabel(gem.name) or gem.name
                info.text = label
    ...
    SetDropdownDisplayText(dropdown, GEM_PLACEHOLDER)
    GemOrder_AttachDropdownTooltips(dropdown, function()
        return GemOrder_GetGemItemId(getSelected())
    end)
    ConfigureOrderDropdown(dropdown, width)
    return dropdown
end
...
function UI:RefreshGemDropdownLabels()
    ...
    for i = 1, 3 do
        ...
        if dropdown and gemName and gemName ~= "None" then
            SetDropdownDisplayText(dropdown, GemOrder_FormatGemDropdownLabel(gemName) or gemName)
        end
    end
end
...
function UI:CreateOrderDialog()
    ...
    local dialog = CreateFrame("Frame", "GemOrderOrderDialog", f)
    ...
    dialog:Hide()
    GemOrder_RegisterEscapeFrame(dialog)
...
function UI:OpenOrderDialog()
    if not GemOrder_HasJoinedWorkshop() then
        print("|cff00ccffGemOrder|r Select a workshop before placing an order.")
        return
    end
...
function UI:CreateOrderForm()
    ...
    f.selectedRole = nil
    f.roleDropdown = CreateFrame("Frame", "GemOrderRoleDropdown", inset, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(f.roleDropdown, function()
        ...
        for _, role in ipairs(GemOrder_ROLES) do
            info = UIDropDownMenu_CreateInfo()
            info.text = role
    ...
    f.submitBtn:SetScript("OnClick", function()
        local _, err = GemOrder_CreateOrder(
            f.selectedGear,
            f.selectedGems,
        ...
        if err then
            ...
            print("|cff00ccffGemOrder|r Order submitted!")
            f.selectedGearCategory = nil
            f.selectedGearClass = nil
            f.selectedGear = nil
...
function UI:UpdateGemRecipeWarning(index)
    ...
    if gemName == "None" or GemOrder_WorkshopHasRecipeForGem(gemName) then
        warningFrame:Hide()
        return
    end
...
function UI:CreateQueueList()
    ...
    local scroll = CreateFrame("ScrollFrame", "GemOrderScroll", inset, "UIPanelScrollFrameTemplate")
...
function UI:CreateGemIconRow(parent, order, yOffset, startX)
    ...
    for i, gemName in ipairs(order.gems or {}) do
        ...
        local itemId = GemOrder_GetGemItemId(gemName)
        if itemId then
            ...
            GemOrder_AttachItemTooltip(btn, function()
                return itemId
            end)
            ...
            GemOrder_AttachItemTooltip(labelBtn, function()
                return itemId
            end)
...
function UI:CreateOrderRow(order, yOffset)
    ...
    title:SetText(string.format(
        "%s  %s  [%s]",
        GemOrder_ColorizePlayer(order.player, order.class),
        GemOrder_GetRoleLabel(order.role),
        GemOrder_GetOrderStatusLabel(order)
    ))
    title:SetWidth(contentWidth - 170)
    title:SetJustifyH("LEFT")

    local canManage = GemOrder_CanManageOrder(order)
    local actionBtn
    ...
    if canManage and order.status ~= "completed" and order.status ~= "cancelled" then
        ...
        if order.status == "pending" then
            ...
            actionBtn:SetScript("OnClick", function()
                GemOrder_UpdateStatus(order.id, "in_progress")
                self:Refresh()
            end)
        elseif order.status == "in_progress" then
            actionBtn = CreateFrame("Button", nil, btnArea, "UIPanelButtonTemplate")
            ...
            actionBtn:SetScript("OnClick", function()
                GemOrder_UpdateStatus(order.id, "completed")
                self:Refresh()
            end)
        end
        ...
        if actionBtn and order.status ~= "completed" then
            ...
            downBtn:SetScript("OnClick", function()
                local ok, err = GemOrder_MoveOrder(order.id, 1)
    ...
    local gearItemId = order.itemId or GemOrder_GetGearItemId(order.item)
    local contentTop = -28
    local contentX = 48
    ...
    if gearItemId then
        ...
        GemOrder_AttachItemTooltip(gearIcon, function()
    ...
    if gearItemId or order.itemLink then
        GemOrder_AttachItemTooltip(itemBtn, function()
            if order.itemLink then
                return order.itemLink
            end
            return gearItemId
    ...
    local canManageWorkshop = room and GemOrder_CanManageWorkshop(room, player)
    ...
    if canManageWorkshop then
        ...
        deleteBtn:SetScript("OnClick", function()
            local ok, err = GemOrder_DeleteOrder(order.id)
            if not ok and err then
                print("|cff00ccffGemOrder|r " .. err)
            end
            self:Refresh()
...
function UI:Refresh()
    if not self.frame or (GemOrder_IsLoggingOut and GemOrder_IsLoggingOut()) then
        return
    end
    ...
    local room = GemOrder_GetActiveRoom()
    if room and GemOrder_IsPromotedJewelcrafter(room, UnitName("player")) then
        self.frame.jcLabel:SetText("|cff00ff00Workshop Jewelcrafter|r")
    else
        self.frame.jcLabel:SetText("")
    end

    if GemOrder_HasJoinedWorkshop() and room then
    ...
    elseif self.activeTab == "stock" and GemOrder_HasJoinedWorkshop() then
        self:RefreshStockPanel()
    elseif self.activeTab == "recipes" and GemOrder_HasJoinedWorkshop() then
        self:RefreshRecipesPanel()
    end
    ...
    if self.activeTab == "orders" and GemOrder_HasJoinedWorkshop() then
        local orders = GemOrder_GetActiveOrders()
        local y = 0
        local lastGroup = nil
    ...
    elseif self.activeTab == "completed" and GemOrder_HasJoinedWorkshop() then
        local orders = GemOrder_GetCompletedOrders()
        local y = 0
        for _, order in ipairs(orders) do
            self:CreateOrderRow(order, y)
            y = y + self:GetOrderRowHeight(order) + ORDER_ROW_GAP
...
function UI:SafeRefresh()
    ...
    if not ok then
        print("|cffff0000GemOrder refresh error:|r " .. tostring(err))
    end
end
