...
local function GetAngle()
    ...
    return GemOrderDB.settings.minimapAngle or 220
end

local function SetAngle(angle)
    GemOrderDB.settings = GemOrderDB.settings or {}
    GemOrderDB.settings.minimapAngle = angle
end
...
function MinimapBtn:Init()
    ...
    local button = CreateFrame("Button", "GemOrderMinimapButton", Minimap)
    ...
    button:SetScript("OnClick", function(_, mouseButton)
        if not GemOrder_EnsureUI() then
            return
        end
        if mouseButton == "RightButton" then
            if GemOrder.UI then
                GemOrder.UI:ShowTab("workshop")
                GemOrder.UI:Show()
            end
            return
        end
    ...
    button:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText("GemOrder", 1, 1, 1)
