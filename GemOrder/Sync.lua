...
function Sync:BroadcastAllOrderQueues(requester)
    for _, room in pairs(GemOrderDB.rooms or {}) do
        if room.id and room.orderQueue and GemOrder_IsRoomMember(room, requester) then
...
function Sync:OnAddonMessage(prefix, message, channel, sender)
    ...
    if msgType == MSG_ADD then
        ...
        local room = roomId and GemOrder_GetRoom(roomId)
        if not room or not GemOrder_IsSameGuild(room) then
            return
        end
        if not GemOrder_IsRoomMember(room, UnitName("player")) then
            return
        end
        local order = {
            id = parts[2],
        ...
        local existing = GemOrderDB.orders[order.id]
        if existing then
    ...
    elseif msgType == MSG_REQUEST then
        ...
        for _, order in pairs(GemOrderDB.orders or {}) do
            if order.roomId then
                ...
                if room and GemOrder_IsSameGuild(room) and GemOrder_IsRoomMember(room, requester) then
                    self:BroadcastOrder(order)
                end
            end
        end
    ...
    elseif msgType == MSG_ROOM then
        ...
        if guild and guild ~= GemOrder_GetGuildName() then
            return
        end
        local existingRoom = GemOrderDB.rooms[parts[2]]
    ...
    elseif msgType == MSG_JOIN then
        ...
        GemOrder_ApplyRoom(room)
        if GemOrder_IsRoomMember(room, UnitName("player")) then
            self:BroadcastRoom(room)
            if GemOrder_CanManageWorkshop(room, UnitName("player")) then
    ...
    elseif msgType == MSG_COLLAB then
        ...
        if room then
            ...
            if parts[3] == "add" then
                ...
                if parts[4] == UnitName("player") and GemOrder_ShouldShareStock() then
    ...
    elseif msgType == MSG_QUEUE then
        local room = GemOrderDB.rooms[parts[2]]
        if room and GemOrder_IsRoomMember(room, UnitName("player")) then
    ...
    elseif msgType == MSG_STOCK then
        ...
        if not room or not GemOrder_HasJoinedWorkshop() then
            return
        end
        if not GemOrder_AcceptsWorkshopStockReport(room, parts[2]) then
            return
        end
        GemOrder_ApplyStockReport(parts[2], DecodeStock(parts[4]), parts[3])
    elseif msgType == MSG_RECIPES then
        ...
        GemOrder_ApplyRecipeReport(parts[2], DecodeRecipeIds(parts[3]))
    end
end
