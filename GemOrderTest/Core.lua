...
local function MigrateSavedData()
    ...
    if (GemOrderTestDB.settings.dataVersion or 0) < DATA_VERSION then
        GemOrderTestDB.orders = {}
        GemOrderTestDB.rooms = {}
        GemOrderTestDB.stock = { bags = {}, bank = {}, jcReports = {} }
        GemOrderTestDB.recipes = { jcReports = {} }
        GemOrderTestDB.settings.activeRoomId = nil
        GemOrderTestDB.settings.dataVersion = DATA_VERSION
    end
end
...
function GemOrderTest_GetStatusLabel(status)
    return STATUS_LABELS[status] or status
end

function GemOrderTest_GetOrderStatusLabel(order)
    if not order then
        return ""
    end
    if order.status == "in_progress" then
        ...
        if assignee then
            return string.format(
                ...
                GemOrderTest_ColorizePlayer(assignee)
            )
        end
    end
...
function GemOrderTest_GetRoleLabel(role)
    role = role or "DPS"
...
function GemOrderTest_GetPlayerClassToken(playerName)
    if not playerName then
        return nil
    end
    local normalized = NormalizePlayerName(playerName)
    GemOrderTestDB = GemOrderTestDB or {}
    GemOrderTestDB.playerClasses = GemOrderTestDB.playerClasses or {}
    if GemOrderTestDB.playerClasses[normalized] then
        return GemOrderTestDB.playerClasses[normalized]
    end

    if normalized == NormalizePlayerName(UnitName("player")) then
        local _, class = UnitClass("player")
        if class then
            GemOrderTestDB.playerClasses[normalized] = class
        end
        return class
    end
...
function GemOrderTest_GetClassColor(classToken)
...
function GemOrderTest_ColorizePlayer(name, classToken)
    classToken = classToken or GemOrderTest_GetPlayerClassToken(name)
    local color = GemOrderTest_GetClassColor(classToken)
    return string.format("|cff%s%s|r", color, name or "")
end

function GemOrderTest_IsValidRole(role)
    for _, valid in ipairs(GemOrderTest_ROLES) do
        if valid == role then
            return true
        end
    end
...
function GemOrderTest_CreateOrder(gear, gems, notes, role)
    ...
    if not GemOrderTest_IsRoomMember(room, player) then
    ...
    if not GemOrderTest_GetGearByName(gear.name) then
    ...
    local order = {
        id = GemOrderTest.Sync:GenerateOrderId(),
        player = player,
        class = classToken,
        item = gear.name,
    ...
    GemOrderTest.Sync:BroadcastOrder(order)
    return order
end

function GemOrderTest_UpdateStatus(orderId, status)
    ...
    GemOrderTest.Sync:BroadcastStatus(orderId, status, order.updatedBy, order.assignedTo)
    return true
end

function GemOrderTest_CancelOrder(orderId)
    local order = GemOrderTestDB.orders[orderId]
    if not order then
        return false, "Order not found."
    end
    ...
    local player = UnitName("player")
    if order.player ~= player and not GemOrderTest_CanManageOrder(order) then
        return false, "You can only cancel your own orders."
    end

    order.status = "cancelled"
    GemOrderTest_RemoveOrderFromQueue(order.roomId, orderId)
    GemOrderTest.Sync:BroadcastStatus(orderId, "cancelled", player)
    return true
end

function GemOrderTest_DeleteOrder(orderId)
    local order = GemOrderTestDB.orders[orderId]
    if not order then
        return false, "Order not found."
    end
    ...
    GemOrderTest.Sync:BroadcastRemove(orderId)
    if order.roomId and room then
        GemOrderTest.Sync:BroadcastOrderQueue(order.roomId, GemOrderTest_GetRoomOrderQueue(room))
    end
    return true
end

function GemOrderTest_GetRoomOrderQueue(room)
    if not room then
        return {}
    end
...
function GemOrderTest_AppendOrderToQueue(roomId, orderId)
    local room = GemOrderTest_GetRoom(roomId)
    if not room or not orderId then
        return
    end
    local queue = GemOrderTest_GetRoomOrderQueue(room)
...
function GemOrderTest_RemoveOrderFromQueue(roomId, orderId)
    local room = GemOrderTest_GetRoom(roomId)
    if not room or not orderId then
        return
    end
...
function GemOrderTest_SyncQueueWithOrders(room)
    if not room then
        return
    end
    local queue = GemOrderTest_GetRoomOrderQueue(room)
    ...
    table.sort(toAdd, function(a, b)
        return (GemOrderTestDB.orders[a].created or 0) < (GemOrderTestDB.orders[b].created or 0)
    end)
    ...
    local cleaned = {}
    for _, id in ipairs(queue) do
        local order = GemOrderTestDB.orders[id]
...
function GemOrderTest_MoveOrder(orderId, direction)
    local room = GemOrderTest_GetActiveRoom()
    if not room then
        return false, "Select a workshop first."
    end
    if not GemOrderTest_IsWorkshopJC(room, UnitName("player")) then
        return false, "Only workshop JCs can reorder the queue."
    end

    GemOrderTest_SyncQueueWithOrders(room)
    ...
    local otherOrder = GemOrderTestDB.orders[otherId]
    ...
    GemOrderTest.Sync:BroadcastOrderQueue(room.id, queue)
    return true
end
...
local function CollectSortedOrders(mode)
    ...
    local list = {}
    local activeRoomId = GemOrderTest_GetActiveRoomId()
    for _, order in pairs(GemOrderTestDB.orders or {}) do
...
function GemOrderTest_GetCompletedOrders()
...
local function OnAddonLoaded(_, addon)
    ...
    local ok, err = pcall(function()
        GemOrderTest.Sync:Init()
        GemOrderTest_TooltipsInit()
        if GemOrderTest_InitStockEvents then
            GemOrderTest_InitStockEvents()
        end
        GemOrderTest.Minimap:Init()
    end)
    if not ok then
    ...
    if GemOrderTest.Minimap.button then
        print("|cff00ccffGemOrderTest|r loaded. Click the gem icon on your minimap, or type |cff00ccff/gemorder|r.")
    else
...
local function TryGuildSync()
    if not IsInGuild() or not GemOrderTest_GetGuildName() then
        return false
    end
    ...
    GemOrderTest_PurgeNonGuildData()
    GemOrderTest.Sync:BroadcastAllRooms()
    GemOrderTest.Sync:RequestSync()

    if GemOrderTest_HasJoinedWorkshop() then
...
local function OnPlayerLogin()
    if not GemOrderTest.Minimap.button then
        pcall(function() GemOrderTest.Minimap:Init() end)
    end
